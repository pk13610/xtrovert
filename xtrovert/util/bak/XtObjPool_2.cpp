...
const xt_size_t _XT_OBJPOOL_CAPACITY_IN_MEMPAGE_ =  1024;
...
int CXtObjPool::Create( xt_size_t nObjCount )
{
	...
	*/
	...
	/*
		Return : object-Count == m_ObjHndBuf.capacity
	*/
	int nRet = FUN_RET_OK;
...
int CXtObjPool::InflatePool( xt_size_t nInflateCount )
{
	...
	*/
	...
	nTotalCount = m_ObjHndBuf.capacity + nInflateCount;
	if ( nTotalCount<m_ObjHndBuf.capacity || nTotalCount<nInflateCount ) /* Overflow */
	{
		return -6;
	}

	/* Create handle buffer. */
	nRet = ReAllocHandleBuf( nTotalCount );
	if ( nRet < 0 )
	{
...
void CXtObjPool::Reset(void)
{
	...
	m_nGrowRealCapacity = grow_capacity_def; /* if (m_ObjHndBuf.capacity==m_nRealCapacity) { AllocChunk(); } */
...
int CXtObjPool::InflateCapacity( xt_size_t nInflateCount )
{
	...
	*/
	...
	if ( upper_capacity > _XT_OBJPOOL_CAPACITY_DEF_ )
	{
		upper_capacity -= _XT_OBJPOOL_CAPACITY_DEF_;
		upper_capacity %= _XT_OBJPOOL_CAPACITY_IN_MEMPAGE_;
		upper_capacity = _XT_OBJPOOL_CAPACITY_IN_MEMPAGE_ - upper_capacity;
	...
	if ( (m_ObjHndBuf.capacity+nInflateCount) >  upper_capacity )
	{
	...
	m_ObjHndBuf.capacity += nInflateCount;
...
void CXtObjPool::OnInflateRealCapacity( xt_poolchk_node_t *pNewChunk )
{
	return;
}

void CXtObjPool::SetupObjGrow(void)
{
	m_nGrowRealCapacity = _xt_poolchk_node_size_avail_ / m_nObjSize;

	m_nGrowRealCapacity = (m_nGrowRealCapacity==0)?1:m_nGrowRealCapacity;
}
