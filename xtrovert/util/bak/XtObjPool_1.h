...
#define XT_OBJ_INIT_COUNT_DEF			(128)

class CXtObjPool
{
public:
	CXtObjPool(void);
	CXtObjPool( xt_size_t nObjSize );/*:m_nObjSize(nObjSize);*/
	virtual ~CXtObjPool(void);

	/* Create object pool with capacity of nObjCount elements */
	int							Init( xt_size_t nObjCount );
	int							InflatePool( xt_size_t nObjCount );
	void						Release(void);
...
#define is_pool_valid			((m_ObjHndBuf.handle_arr) && (m_ObjHndBuf.fn_cmp))
	...
	int							ReAllocHandleBuf( xt_size_t nObjCount );
	...
	virtual xt_size_t			GetObjGrow(void)										{ return m_nObjGrow; }
...
#define grow_capacity_def			(128)		/*m_nObjGrow*/
